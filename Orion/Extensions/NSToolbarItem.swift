//
//  NSToolbarItem-Identifier.swift
//  Orion
//
//  Created by Corban Amouzou on 2022-01-03.
//

import Foundation
import Cocoa

extension NSToolbarItem.Identifier {
    static let addTab: NSToolbarItem.Identifier = .init("AddTab")
    static let reload: NSToolbarItem.Identifier = .init("Reload")
    static let addBookmark: NSToolbarItem.Identifier = .init("AddBookmark")
    static let pinTab: NSToolbarItem.Identifier = .init("PinTab")
    static let tabs: NSToolbarItem.Identifier = .init("Address")
    static let backwordForwardGroup: NSToolbarItem.Identifier = .init("BackForwardGroup")
}
